 ...
 *   - generate-claude-profile: Developer Profile section in .trae/rules/
 *   - generate-claude-md: full .trae/rules/ with managed sections
 */
...
const CLAUDE_MD_FALLBACKS = {
  ...
  skills: 'No project skills found. Add skills to any of: `.trae/skills/`, `.agents/skills/`, `.cursor/skills/`, or `.github/skills/` with a `SKILL.md` index file.',
};
 ...
 * (name + description) for each. Returns a table summary for .trae/rules/ so
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = path.join(os.homedir(), '.claude', '.trae/rules/');
  } else if (options.output) {
    targetPath = path.isAbsolute(options.output) ? options.output : path.join(cwd, options.output);
  } else {
    // Read claude_md_path from config, default to ./.trae/rules/
    let configClaudeMdPath = './.trae/rules/';
    try {
